...
export async function snapshotErData(db: any): Promise<void> {
  try {
    ...
    // Best-effort background work — log at warn level for observability.
    process.stderr.write(JSON.stringify({ level: 40, time: Date.now(), msg: `[er-snapshot] failed: ${err instanceof Error ? err.message : String(err)}` }) + "\n");
  }
}
